...
- 发布页支持图片、标题、描述、分类、校内地图点选、地点搜索和手动确认。
...
发布页支持三种地点确认方式：用户可以在内置校内地图上点击地点标记，也可以搜索上科大校内地点或从常用地点中手动选择。
...
当前版本不调用定位接口，不采集实时位置、Wi-Fi、蓝牙、设备信标或腾讯地图室内定位信号。地图只展示上科大校内地点库中的固定标记，用户点击标记后手动确认发布地点。
...
当前版本不调用位置、Wi-Fi、蓝牙或室内定位相关私有接口，也不依赖第三方地图插件。
