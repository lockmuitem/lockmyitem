...
- 发布页支持图片、标题、描述、分类、获取位置、校内地图点选、地点搜索和手动确认。
...
发布页支持多种地点确认方式：用户可以手动触发“获取位置”，也可以在内置校内地图上点击地点标记，或搜索上科大校内地点后手动确认。
...
当前版本不调用普通定位接口。只有用户点击“获取位置”按钮时，才会采集 Wi-Fi/BLE 信号并调用云函数尝试解析；云函数优先使用百度智能硬件定位，未配置或失败时再回退到高德智能硬件定位、腾讯地图网络定位。地图只展示上科大校内地点库中的固定标记，用户点击标记后手动确认发布地点。
...
当前版本不调用普通定位接口，也不依赖第三方地图插件。“获取位置”会在用户点击后调用 Wi-Fi/BLE 相关接口；提交审核前需要确保微信后台隐私声明与实际调用一致。

## 已知限制

- 图像识别依赖腾讯云混元 API，未配置 `HUNYUAN_API_KEY` 或腾讯云 `SecretId/SecretKey` 时会返回 `MODEL_NOT_CONFIGURED`。
- “获取位置”依赖云函数环境变量 `BAIDU_LOC_KEY`、`AMAP_KEY` 或 `TENCENT_MAP_KEY`。三者都不配置时只保留地图选择和地点搜索。
