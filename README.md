...
- 发布页支持图片、标题、描述、分类、校内地点搜索、腾讯地图选点和手动确认。
...
- `pages/publish/publish`：发布招领或寻物，支持腾讯地图选点、校内地点选择和相似物品提醒。
...
发布页支持两种地点确认方式：用户可以打开腾讯地图选点插件选择位置，也可以搜索上科大校内地点或从常用地点中手动选择。
...
当前版本的小程序业务代码不直接调用定位接口，不采集 Wi-Fi、蓝牙、设备信标或腾讯地图室内定位信号。地图点位由腾讯地图选点插件提供，返回后会映射到最近的上科大校内地点并要求用户确认。

腾讯地图选点插件需要在微信公众平台后台添加“腾讯位置服务地图选点”插件，并在云函数环境变量中配置腾讯位置服务 Key。
...
当前版本不调用 Wi-Fi、蓝牙或室内定位相关私有接口。若启用腾讯地图选点插件，需要在微信公众平台插件管理中添加“腾讯位置服务地图选点”，并按插件要求保留地图选点所需的位置授权说明。
