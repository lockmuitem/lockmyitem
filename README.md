...
- `pages/publish/publish`：发布招领或寻物，支持校内地点选择和相似物品提醒。
...
发布页不调用用户位置接口。用户可以搜索上科大校内地点，也可以从常用地点中手动选择并确认，避免申请额外权限。
...
当前版本不采集用户实时位置、Wi-Fi、蓝牙、设备信标或腾讯地图室内定位信号。发布地点完全由用户在校内地点库中手动选择。
