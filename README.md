...
- 发布页支持图片、标题、描述、分类、室内增强测试、校内地图点选、地点搜索和手动确认。
...
发布页支持多种地点确认方式：用户可以手动触发室内增强测试，也可以在内置校内地图上点击地点标记、搜索上科大校内地点或从常用地点中手动选择。
...
当前版本不调用普通定位接口。只有用户点击“室内增强”按钮时，才会采集 Wi-Fi/BLE 信号并调用云函数尝试解析；地图只展示上科大校内地点库中的固定标记，用户点击标记后手动确认发布地点。
...
当前版本不调用普通定位接口，也不依赖第三方地图插件。室内增强测试会在用户点击后调用 Wi-Fi/BLE 相关接口；提交审核前需要确保微信后台隐私声明与实际调用一致。
